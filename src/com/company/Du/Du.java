...
public class Du {
    ...
    public static void signCrypt(String pairFile, String publicFile, String pidFile, String skFile, String pkFile,  String message, String signCryptFile) throws NoSuchAlgorithmException, IOException {
        ...
        byte [] hi_hash = sha1( pidi+Ti.toString()+P_pub.toString()+P.toString());
        ...
        byte [] hj_hash = sha1( pidj +Tj.toString()+P_pub.toString()+P.toString());
        ...
        Element V = (Qj.add(Tj).add(P_pub.powZn(hj))).powZn(u.mul(xi.add(di)));
        byte [] K_hash = sha1( pidj +U_.toString()+V.toString());
        ...
        for (int j = 0; j < messageByte.length; j++){
            c[j] = (byte)(messageByte[j] ^ K_hash[j]);
        }
        Element k = bp.getZr().newRandomElement().getImmutable();
      //  Element I = P.powZn(k).getImmutable();
        //  System.out.println(I);

        byte[] h3_hash = sha1(K_hash.toString()+pidj+Qj.toString()+Tj.toString());
        Element e = bp.getZr().newElementFromHash(h3_hash,0, h3_hash.length);


        byte[] h4_hash = sha1(pidi+Qi.toString()+Ti.toString()+U_.toString());
        Element l = bp.getZr().newElementFromHash(h4_hash,0, h4_hash.length);

        byte[] h5_hash = sha1(pidi+Qi.toString()+Ti.toString()+U_.toString()+e.toString()+c.toString());
        Element h5 = bp.getZr().newElementFromHash(h5_hash,0, h5_hash.length);

        Element tha = (u.mul(xi.add(di))).div(l.mul(xi).add(h5.add(di)));

       // Element tha = hi.mul(xi).add(hj.mul(di)).mul(r);

        Properties sigC = new Properties();
        sigC.setProperty("U_", Base64.getEncoder().encodeToString(U_.toBytes()));
       // sigC.setProperty("e", Base64.getEncoder().encodeToString(.toBytes()));
    ...
    public static void unsignCrypt(String pairingFile, String publicFile, String pidFile ,String skFile, String pkFile, String signCryptFile, String message) throws NoSuchAlgorithmException, UnsupportedEncodingException {
      ...
      //  String IStr = sigC.getProperty("I");
       ...
       // Element I = bp.getG1().newElementFromBytes(Base64.getDecoder().decode(IStr)).getImmutable();
        ...
        Element Z = U_.powZn(xj.add(dj));

        //System.out.println(Z);


        byte [] K = sha1(pidj+U_.toString()+Z.toString());

      //  Element I_ = P.powZn(tha).sub(Qi.powZn(r.mul(hi))).sub(Ti.powZn(r.mul(hj))).sub(P_pub.powZn(r.mul(hi.mul(hj))));
        // System.out.println(I_);

        byte [] h3_hash = sha1(K.toString()+pidj+Qj.toString()+Tj.toString());
        Element e = bp.getZr().newElementFromHash(h3_hash,0, h3_hash.length);
       ...
       Element W = Qi.powZn(l.mul(tha)).add(P_pub.powZn(hi.mul(h5.mul(tha)))).add(Ti.powZn(h5.mul(tha)));
